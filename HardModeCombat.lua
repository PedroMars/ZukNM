...
local function getHighestPriorityTarget()
    ...
    local valor = API.GetAllObjArray1({POSSIBLE_TARGETS.Mejkot}, 2, { 1 })
    local valortotal = #valor

    if valortotal >= 1 then
        local npcs = valor
        for _, npc in ipairs(npcs) do
            if npc.Life > 0 then
                local animFactor = npc.Anim > 0 and 3 or 1
...
local function findNextBestTarget()
    ...
    if FIGHT_STATE.wave == 18 then
        ...
        if zuk.Anim == 34505 and areTargetsAlive({ POSSIBLE_TARGETS.Igneous_Hur, POSSIBLE_TARGETS.Igneous_Mej, POSSIBLE_TARGETS.Igneous_Xil }) and main:needsTarget() then
...
function main:TemHordaDeNPCsMeAtacando()
    -- Obtém a lista de todos os NPCs que estão interagindo (atacando) com o jogador local.
    -- Os parâmetros (false, 0) são valores padrão para não filtrar por animação ou pontos de vida.
    local npcsAtacando = API.OthersInteractingWithLpNPC(true, 0)

    -- Verifica se a tabela retornada é válida e não nula.
    if npcsAtacando then
        -- O operador '#' retorna o número de elementos em uma tabela (array).
        -- Comparamos se a contagem é maior ou igual a 3.
        if #npcsAtacando >= 4 then
            return true
        end
    end

    -- Retorna false se a tabela for nula ou se a contagem for menor que 3.
    return false
end
...
local function goToSafespot(safespot)
    ...
    local medo = main:evade_lavaskill()

    if playerPos.x == safespot.x and playerPos.y == safespot.y then
        return
    end
    if not main:escaparDoPerigo() then
        if API.DoAction_Tile(safespot) then
            ...
            API.WaitUntilMovingEnds(3, 7)
            local nextTarget = findNextBestTarget() -- Força a reavaliação
            if nextTarget ~= nil then
                attackTarget(nextTarget) -- Tenta atacar o novo alvo
            end
        end
...
function main:needsNewTarget()
    return main:needsTarget() or shouldStopTargetingZuk() or shouldStopTargetingAken()
end
...
local toaquispot4 = false
local toaquispot5 = false
function main:Wave8 ()
    local numeroDeAlvos =  API.GetAllObjArray1({28539}, 80, { 1 })
    local valortotal = #numeroDeAlvos
    local numeroDeAlvos2 = API.GetAllObjArray1({28536}, 3, { 1 })
    local valortotal2 = #numeroDeAlvos2



    if valortotal >= 3  then
        if not IsPlayerAtWPoint(SAFESPOT4,3) and not toaquispot4  then
            print(valortotal)
            goToSafespot(SAFESPOT4)
            API.RandomSleep()
        else
            toaquispot4 = true
            local nextTarget = findNextBestTarget() -- Força a reavaliação
    ...
    elseif valortotal < 3 and valortotal >= 2 and (FIGHT_STATE.wave == 7 or FIGHT_STATE.wave == 8) and not toaquispot5 then
        if not IsPlayerAtWPoint(SAFESPOT5,3) then
            goToSafespot(SAFESPOT5)
            print(valortotal)
            API.RandomSleep()
    ...
    main:needsNewTarget()
        elseif valortotal2 >= 2  then
        if not IsPlayerAtWPoint(SAFESPOT4,3)  then
            print(valortotal)
            goToSafespot(SAFESPOT4)
            API.RandomSleep()
        else
...
function main:useAbility(abilityName)
    ...
    if main:needsNewTarget() then return false end
...
function main:DefesaMej()
    local valor = API.GetAllObjArrayFirst({ 28536 }, 10, { 1 })
    if FIGHT_STATE.isNormalWave then
    if valor ~= nil and valor.Anim == 16102 then
            if attackTarget(valor) then
                return true
            end
...
function main:escaparDoPerigo()
    -- 1. Encontra o tile seguro para escapar.
    --    Usa um nome de variável mais descritivo.
    local tileDeFuga = main:evade_lavaskill()

    -- 2. Se não houver para onde fugir, encerra a função imediatamente.
    if not tileDeFuga then
        return false -- Retorna 'false' para indicar que nenhuma ação foi tomada.
    end
    -- 3. Tenta usar a habilidade de maior prioridade: Dive.
    --    Dive é rápido e preciso.
    if API.DoAction_TileF(tileDeFuga) then
        API.logInfo("Escapei do perigo usando Dive!")
        API.RandomSleep2(800, 200, 100)
        API.RandomSleep2(300, 500, 300)
        local nextTarget = findNextBestTarget() -- Força a reavaliação
        if nextTarget ~= nil then
            attackTarget(nextTarget) -- Tenta atacar o novo alvo
        end
    end
        -- API.RandomSleep2(200, 100, 50) -- Se precisar de um sleep, que seja mínimo.
        return true -- Retorna 'true' para indicar que uma ação de fuga foi bem-sucedida.
...
local function manageBuffs()
    ...
    local necroSkill = API.GetSkillByName("NECROMANCY")
    ...
    if hp < math.random(6500, 8000) then
    ...
    if necroSkill.boostedLevel < 115 then
        if  API.DoAction_Ability_check(FOOD_POT_NAME, 1, API.OFF_ACT_GeneralInterface_route, true, true, false) then
            API.RandomSleep2(500, 10, 10)
        end
    end

    if hppet < math.random(10000, 13000) and hppet>0 then
...
local function waveClearRotation()
    ...
    if not FIGHT_STATE.wave ~= 13 then
...
local function doRotation()
    ...
    elseif FIGHT_STATE.isIgneousWave then
        ...
        elseif areTargetsAlive({ POSSIBLE_TARGETS.Igneous_Xil, POSSIBLE_TARGETS.Igneous_Mej }) then
            return thresholdRotation()
        elseif extraActionButtonVisible()  and  adren < (HAS_ZUK_CAPE and 20 or 100) then
            return buildAdrenRotation()
        elseif extraActionButtonVisible() and  adren >= (HAS_ZUK_CAPE and 20 or 100) then
...
---------------------------------------------------------------------
local toaquispot2 = false
local toaquispot3 = false
local function IgneousHardMode()
    if areTargetsAlive({ POSSIBLE_TARGETS.Igneous_Hur }) then
    elseif areTargetsAlive({ POSSIBLE_TARGETS.Igneous_Xil }) then
        if not IsPlayerAtWPoint(SAFESPOT2,1) and not toaquispot2 then
            goToSafespot(SAFESPOT2)
        else
            toaquispot2 = true
        end
    elseif areTargetsAlive({ POSSIBLE_TARGETS.Igneous_Mej }) and not toaquispot3 then
        if not IsPlayerAtWPoint(SAFESPOT3,2) then
            goToSafespot(SAFESPOT3)
        else
            toaquispot3 = true
        end
    else
...
local function findArenaCoords()
    ...
    if zuk ~= nil then
        ...
        SAFESPOT4 = WPOINT.new(zukX + 13, zukY -32, 0) -- alterar valores
        SAFESPOT5 = WPOINT.new(zukX + 9, zukY -36, 0) -- alterar valores
        ARENA_MIN_X = zukX - 15
        ARENA_MAX_X = zukX + 15
        ARENA_MAX_Y = zukY - 4
        ARENA_MIN_Y = zukY - 35
...
local function updateFightState()
    ...
    FIGHT_STATE.isIgneousSafe = Ig_WAVES[currWave] or false
...
while API.Read_LoopyLoop() do
    ...
    if not areTargetsAlive(ALL_POSSIBLE_TARGET_IDS) then
        if FIGHT_STATE.isIgneousSafe or FIGHT_STATE.isJadWave  and SAFESPOT_JAD ~= nil then
    ...
    main:escaparDoPerigo()
    if not areTargetsAlive(ALL_POSSIBLE_TARGET_IDS) then
        if HardMode and (FIGHT_STATE.isNormalWave) and not FIGHT_STATE.isIgneousSafe and SAFESPOT3 ~= nil then
            goToSafespot(SAFESPOT3)
            API.WaitUntilMovingEnds(2, 3)
            API.logWarn("[SEAR] Moving to safe point")
            API.logWarn("Valor atual da variável tentativas: " .. tentativas, "info")
            while not IsPlayerAtWPoint(SAFESPOT3,0) and tentativas<6 and wave~=18  do
                main:escaparDoPerigo()
                goToSafespot(SAFESPOT3)
                API.WaitUntilMovingEnds(1, 3)
                tentativas = tentativas + 1
    ...
    main:escaparDoPerigo()
    -- Handle targeting
    if main:needsNewTarget() or FIGHT_STATE.isPizzaPhase then
    ...
    if zukPreparation:HandleDeathNPC() then
        if SAFESPOT_JAD == nil then
            findArenaCoords()
            while naoPegou and wave~=18 and tentativas<6  do
                findArenaCoords()
    ...
    if HardMode and FIGHT_STATE.isIgneousWave then
        IgneousHardMode()
    end
    main:Wave8()
    main:vidaZUk()
